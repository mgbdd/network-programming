...
public class ProxyServer implements Runnable {
    ...
    private void connect(SelectionKey key) throws IOException {
        ...
        attachment.in.put(Constants.CONNECTION_OK_REPLY).flip();
    ...
    private void scanAndReplyAuthRequest(SelectionKey key) throws Socks5Exception {
        ...
        for (int method_i = 0; method_i < methods_count; method_i++) {
            int method = data[method_i + 2];
            if (method == Constants.NO_AUTH) {
        ...
        attachment.out.put(Constants.AUTH_NO_AUTH_REPLY).flip();
    ...
    private void scanAndReplyConnectionRequest(SelectionKey key) throws Socks5Exception, IOException {
        ...
        if (data[1] != Constants.CONNECTION_COMMAND) {
            throw new Socks5Exception("0x%02x command is not supported, only 0x01 (connect) is supported", 1);
        }

        if (data[3] == Constants.ADDR_TYPE_IPV4) {
            byte[] connectAddrBytes = new byte[]{data[4], data[5], data[6], data[7]};
            InetAddress connectAddr = InetAddress.getByAddress(connectAddrBytes);
            int portPos = 8;
            int connectPort = ((data[portPos] & 0xFF) << 8) + (data[portPos + 1] & 0xFF);

            registerPeer(connectAddr, connectPort, key);
            key.interestOps(0);
        } else if (data[3] == Constants.ADDR_TYPE_HOST) {
            int hostLen = data[4];
            int hostStart = 5;
            ...
            String host = new String(Arrays.copyOfRange(data, hostStart, hostStart + hostLen));
            int portPos = hostStart + hostLen;
            int connectPort = ((data[portPos] & 0xFF) << 8) + (data[portPos + 1] & 0xFF);
